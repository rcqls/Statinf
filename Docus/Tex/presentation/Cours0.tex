...
\only<7-8>{
\pgfputat{\pgfxy(0.5636363636363637,0.07242835407792879)}{\pgfbox[center,center]{
{\scriptsize \only<7>{{\color{orange}$\Est{\delta_{d_{\sigma^2},0}}{y^C,y^D}\stackrel{R}{=}$\texttt{(var(yC)-var(yD))/seDVar(yC,yD)=0.256}}
$\left\{\begin{array}{l}
\nless{\color{darkgreen}\delta_{lim,2.5\%}^-\simeq -1.96}\\
\ngtr{\color{darkgreen}\delta_{lim,2.5\%}^+\simeq 1.96}
\end{array}\right.$
}
\only<8>{{\color{orange}$p-valeur (bi)\stackrel{R}{=}$ \texttt{2*(1-pnorm(var(yC)-var(yD))/seDVar(yC,yD))=79.81\%}}$\nless${\color{darkgreen}$\alpha=5\%$}}
}}}}
